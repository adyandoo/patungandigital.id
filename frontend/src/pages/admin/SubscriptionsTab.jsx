  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        ...
        <button data-testid="admin-add-sub" className="brutal-btn brutal-btn-red" onClick={() => { setEditing(null); setShowModal(true); }}><PlusCircle weight="bold" /> Tempatkan User</button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-black text-white">
            <tr>
              ...
              <th className="text-left px-4 py-3 font-mono uppercase text-xs">Sampai</th>
          ...
          <tbody data-testid="subs-table">
            {sorted.map((s) => (
              <tr key={s.id} className="border-t-2 border-black" data-testid={`sub-row-${s.id}`}>
                ...
                <td className="px-4 py-3">{s.end_date ? new Date(s.end_date).toLocaleDateString("id-ID") : "-"}</td>
                <td className="px-4 py-3">{rupiah(s.price)}</td>
                <td className="px-4 py-3"><span className="pd-tag">{s.status}</span></td>
                <td className="px-4 py-3 flex gap-2">
                  <button onClick={() => { setEditing(s); setShowModal(true); }} className="brutal-sm p-2 bg-[#007AFF] text-white" title="Edit tanggal & durasi" data-testid={`sub-edit-${s.id}`}><PencilSimple weight="bold" /></button>
                  <button onClick={() => del(s.id)} className="brutal-sm p-2 bg-[#FF3B30] text-white" data-testid={`sub-delete-${s.id}`}><Trash /></button>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
      ...
      {showModal && <SubModal sub={editing} users={users} services={services} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); load(); }} />}
    </div>
  );
}

function SubModal({ sub, users, services, onClose, onSaved }) {
  const isEdit = !!sub;
  const [form, setForm] = useState({
    user_id: sub?.user_id || "", service_id: sub?.service_id || "",
    role: sub?.role || "regular", group_id: sub?.group_id || "",
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.patch(`/admin/subscriptions/${sub.id}`, payload);
  ...
  return (
    <Modal onClose={onClose} title={isEdit ? "Edit Subscription (tanggal, durasi, harga)" : "Tempatkan User ke Service"}>
      <form onSubmit={save} className="space-y-3" data-testid="sub-modal-form">
        <F label="User">
          <select required disabled={isEdit} className="brutal-input" value={form.user_id} onChange={(e) => setForm({ ...form, user_id: e.target.value })} data-testid="submod-user">
            <option value="">Pilih user...</option>
            {users.filter((u) => u.role === "user").map((u) => <option key={u.id} value={u.id}>{u.name} ({u.email})</option>)}
          </select>
        </F>
        <F label="Service">
          <select required disabled={isEdit} className="brutal-input" value={form.service_id} onChange={(e) => { const svc = services.find((s) => s.id === e.target.value); setForm({ ...form, service_id: e.target.value, price: svc?.price_regular || 0, group_id: "" }); }} data-testid="submod-service">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <F label="Status">
            <select className="brutal-input" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} data-testid="submod-status">
          ...
          <F label="Harga (Rp)"><input type="number" className="brutal-input" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} data-testid="submod-price" /></F>
          <F label="Durasi (bulan)"><input type="number" min={1} max={24} className="brutal-input" value={form.duration_months} onChange={(e) => setForm({ ...form, duration_months: e.target.value })} data-testid="sub-modal-duration" /></F>
        </div>
        {!isEdit && (
        ...
        <div className="grid grid-cols-2 gap-3">
          <F label="Batas awal langganan"><DatePicker value={form.start_date} onChange={(v) => setForm({ ...form, start_date: v })} testId="sub-modal-start" placeholder="Pilih tanggal mulai" allowClear={false} /></F>
          <F label="Batas akhir langganan"><DatePicker value={form.end_date} onChange={(v) => setForm({ ...form, end_date: v })} testId="sub-modal-end" placeholder="Pilih tanggal berakhir" /></F>
        </div>
        <button type="submit" className="brutal-btn brutal-btn-red" data-testid="submod-save">{isEdit ? "Simpan Perubahan" : "Simpan"}</button>
      </form>
    </Modal>
  );
}
